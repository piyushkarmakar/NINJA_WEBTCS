...
public void TC005() throws IOException, InterruptedException 
	{
		...
		if(wd1.findElement(By.xpath("/html/body/div[2]/div/div/div/div[1]/div/h2")).isDisplayed()) {
			...
			Files.copy(src,new File("./Screenshots/LaunchApp9.png"));
		}
		else {
		...
		wd1.findElement(By.name("email")).sendKeys(p.getProperty("unm5"));
		Thread.sleep(2000);
		wd1.findElement(By.name("password")).clear();
		wd1.findElement(By.name("password")).sendKeys(p.getProperty("pwd5"));
		Thread.sleep(2000);
		...
		if(Actual.equalsIgnoreCase(Expect)) {
			...
			File src =((TakesScreenshot)wd1).getScreenshotAs(OutputType.FILE);
			Files.copy(src,new File("./Screenshots/no_uname&PWD.png"));
		}
		else {
